...
function new_addcart2(url){
    $.post(url,{"a":1},function(data){
        alert(data);

        var obj	= eval("("+data+")");

        $(".cart_total_items").html(obj.sum);
        $("#cart_total").html('￥'+obj.cartMoney);
        var item_id		=	'cart_items_'+obj.cartDetailsId;
        var html=	'';

        html	+=	'<tr id="'+item_id+'">';
        html	+=	'<td class="table-padding">';
        html	+=	'<div class="mui-pic">';
        html	+=	'<a href="javascript:remove_cart2(\'${pageContext.request.contextPath}/deleteCartGoods\',\''+obj.cartDetailsId+'\')" class="close_btn"><img src="/static/picture/close3.png" /></a>';
        html	+=	'<a href="javascript:void(0)"><img src="/static/picture/'+obj.goodsPicture+'"  width="74" height="56"/></a></div></td>';
        html	+=	'<td><p><a href="javascript:void(0)">'+obj.goodsName+'</a></p></td>';
        html	+=	'<td><em>￥ '+obj.goodsPrice+'</em></td>';
        html	+=	' </tr>';
...
function new_addcart3(url){
    $.post(url,{"a":1},function(data){
        var obj= new Array();
        obj=data.split(",");

        $(".cart_total_items").html(obj[0]);
        $("#cart_total").html('￥'+obj[1]);
        var item_id		=	'cart_items_'+obj[2];
        var html=	'';

        html	+=	'<tr id="'+item_id+'">';
        html	+=	'<td class="table-padding">';
        html	+=	'<div class="mui-pic">';
        html	+=	'<a href="javascript:remove_cart2(\'deleteCartGoods\',\''+obj[2]+'\')" class="close_btn"><img src="/static/picture/close3.png" /></a>';
        html	+=	'<a href="javascript:void(0)"><img src="/static/picture/'+obj[3]+'"  width="74" height="56"/></a></div></td>';
        html	+=	'<td><p><a href="javascript:void(0)">'+obj[4]+'</a></p></td>';
        html	+=	'<td><em>￥ '+obj[5]+'</em></td>';
        html	+=	' </tr>';
